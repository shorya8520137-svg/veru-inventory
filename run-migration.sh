...
# Self Transfer Migration Script
# Run this on your server to create the missing tables

echo "Running Self Transfer Migration..."
echo "=================================="

mysql -h api.giftgala.in -u inventory_user -pStrongPass@123 inventory_db << 'EOF'
...
CREATE INDEX IF NOT EXISTS idx_timeline_entity_date ON timeline_events(entityType, entityId, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_transfer_status_date ON inventory_transfers(transferStatus, created_at DESC);

-- Verify tables were created
SHOW TABLES LIKE 'inventory_%';
SHOW TABLES LIKE 'timeline_%';
