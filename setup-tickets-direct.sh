...
echo "🎫 Direct Ticket Management Database Setup"
echo ""

# Direct MySQL command to setup tables
echo "Setting up ticket management tables..."
echo "Please enter your MySQL password when prompted:"

mysql -h 127.0.0.1 -P 3306 -u inventory_user -p inventory_db << 'EOF'
...
-- Insert sample data
INSERT IGNORE INTO tickets (ticket_number, title, description, priority, status, category, assigned_to, created_by) VALUES
...
INSERT IGNORE INTO ticket_followups (ticket_id, comment, comment_type, created_by) VALUES
...
UPDATE tickets SET status = 'Resolved', resolved_at = NOW() WHERE ticket_number = 'TKT-005';

-- Show results
SELECT 'Tables created successfully!' as Status;
SELECT COUNT(*) as 'Total Tickets' FROM tickets;
SELECT COUNT(*) as 'Total Follow-ups' FROM ticket_followups;

EOF

echo ""
echo "✅ Ticket Management Database Setup Complete!"
echo ""
echo "🔧 Next steps:"
echo "1. Your server should automatically detect the new routes"
echo "2. Navigate to /tickets in your browser"
echo "3. Test the ticket management system"
echo ""