...
export const bulkUploadAPI = {
  ...
  async getWarehouses() {
    try {
      ...
      const warehouses =
        response.warehouses || response.data?.warehouses || response.data || [];
      ...
      console.warn(
        "Warehouse management API unavailable, falling back to bulk-upload warehouses:",
        error?.message,
      );
    }

    const fallback = await apiRequest("/api/bulk-upload/warehouses");
    const warehouses =
      fallback.warehouses || fallback.data?.warehouses || fallback.data || [];
    return {
      ...fallback,
        ...
        headers.forEach((header, headerIndex) => {
          ...
          if (normalizedHeader === "qty" || normalizedHeader === "quantity") {
            ...
            if (row.qty < 0 && value !== "" && !value.includes("-")) {
              console.warn(
                `Unexpected negative qty conversion at row ${index + 1}:`,
                {
                  original: value,
  ...
  validateRow(row) {
    ...
    if (row.qty === undefined || row.qty === null || row.qty === "") {
      ...
      if (isNaN(qty)) {
        errors.push(
          `Quantity must be a valid number, got: "${row.qty}" (type: ${typeof row.qty})`,
        );
      }
